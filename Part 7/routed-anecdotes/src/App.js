...
const Anecdote = ({ anecdote }) => (
	<div>
		...
		<p>
			For more info, see <a href={anecdote.info}>this link</a>
		</p>
	</div>
);
...
const Created = withRouter(CreateNew);
...
const App = () => {
	...
	const notify = anecdoteName => {
		setNotification(`A new anecdote ${anecdoteName} created`);
		setTimeout(() => {
			setNotification("");
		}, 10000);
	};
	const showNotifiation = () => {
		...
		<div>
			...
			<Router>
				...
				<Route
					exact
					path='/create'
					render={() => <Created notify={notify} addNew={addNew} />}
				/>
